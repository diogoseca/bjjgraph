...
| `npm run calibrate` | **Calibrated probability elicitation (dry-run).** An expert panel (Danaher, Gordon Ryan, Craig Jones, Roger Gracie, a BJJ-Fanatics generalist), each elicited in a SEPARATE Claude call, estimates occurrence% / success% / outcome-distribution per technique. Mechanism-first (which principles attacker leverages vs defender fails to leverage), comparative ranking + anchored to known-% references. Aggregates IN CODE (relevance × inverse-variance, bootstrap bagging, spread→pseudo-count), then **de-biases the single-LLM central-tendency compression** by fitting the inverse from the run's own blind anchor probes (`true=(elicited-b)/a`, high-side damped, clipped) — restoring the high tail (dominant finishes) the raw panel suppresses. Per-band (low/mid/high) anchor MAE is reported so a hidden tail bias can't pass behind a low pooled MAE. Writes `calibration_results.json` + `calibration_proposals.json`; changes nothing else. Treats the panel as a *prior*, not an independent crowd. Use `--reaggregate` to re-run aggregation+de-bias on an existing results file with NO Claude calls; `--no-debias` to disable the correction. |
| `npm run calibrate:apply` | Fold the calibrated **success-rate prior** into `templates/votes.json` (only for entries still at the pure seed `vote_count==30`; never clobbers real votes). Gated on the anchor-recovery self-check unless `--force`. **Out-of-band (extrapolated) entries** — corrected success outside the trusted [15,77] band, where the anchors don't constrain the slope — are HELD for human review (`needs_human_review` in the proposals) and only the confident mid-band auto-applies; pass `--apply-out-of-band` to include them. occurrence% / outcomes stay in the human-gated `calibration_proposals.json`. |
