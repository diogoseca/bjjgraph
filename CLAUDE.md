...
- Mobile: ONE 88vw drawer (the two competing drawers are gone); the `.ng-drilltab` pill is unchanged.
- **Account chip (v1.93.0): BOTTOM-right** (desktop `bottom:24;right:24`; phone `bottom:86;right:14`, stacked above the thumb-band pill). It still fades while the pane is open — the pane covers that corner on both form factors. There is deliberately **NO account menu** (forward catalog pins `.ng-account-menu` count 0): the chip opens the pane's History, and the pane's bottom anchor + footer (Settings ⚙ · Terms · Privacy, plus Log out in the History foot when signed in) ARE the account surface. The Settings modal also carries Terms · Privacy (`[data-settings-legal]`).
