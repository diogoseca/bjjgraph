...
- **Pane bottom anchor (v1.93.0, slimmed v1.95.0, one-line v1.97.0):** the guest save nudge — ONE dense row (`[data-anchor-authrow]`): plain "Save your progress —" + the **Create account** pill + a quiet "Log in" link, 44px hit areas, nowrap at the 360px rail — is ONE block (`.ng-pane-anchor`, `[data-anchor-auth]`/`[data-anchor-login]`) at the pane's foot, directly above the Settings/Terms/Privacy row — visible on all three tabs, hidden during study takeover and collapsed entirely when signed in (it is then contentless). The mastered/today/weak-spots stat row moved to the TOP of Explore's body (`_exploreStatsRow`, `[data-explore-stats]`, same `.ngStat` handles) — "the weak spots are the call to action" there (owner, v1.95.0). The History head is empty (the drill head is study-only now); signed-in users keep their session CTA + Log out in the Last rolls foot.
- **Two belts, two meanings (v1.95.0 — do not conflate):** the woven **knowledge belt** (top of Explore's body since v1.96.0) renders `gameScore()` — and WHITE IS THE FLOOR, never a target: the cold state wears the white belt whose displayed road spans the whole 0→blue stretch ("there is never 0% to white. It's always 0% to blue", owner; `gameScore()`/`BELT_SCORE` math untouched — display semantics only; white's tape stripes are quarter-marks of that displayed road, held belts wear `gameScore().stripes` exactly). The **Challenges-tab mini belt** (`.ng-tab-belt`, `data-tab-stripes`) wears the PINNED track's color with 0-4 stripes from that track's PROVEN-UNITS fraction (`unitComplete`: lessons done + checkpoint — v1.95.3; the objectives fraction was too generous, coach auto-ticks gave a guest unearned stripes) — LADDER progress, deliberately NOT `gameScore().stripes`.
...
- **Lists (v1.97.0):** the `+` beside "Your lists (N)" (`[data-lists-new]`, aria "New list", 44px) is THE deliberate list-creation control (it replaced the static "share a class" caption; `newList()` stays the one creation function — default name "Class · <date>", newborn becomes the active add target and its row highlights; per-list Share untouched; no `?capture=today` deep link exists in source). Explore's `keepList` reset-survival accepts owned lists (even empty) AND the `__shared` preview.
...
- Mobile: ONE 88vw drawer (from the LEFT edge since v1.94.0; strip-tap dismiss on the exposed right strip unchanged); the `.ng-drilltab` pill is unchanged.
...
- Rendered as the accessible `.ng-knowledge-meter` (role=meter; aria carries rank, stripes, road) inside `[data-knowledge]` at the top of Explore's body (v1.96.0 — the `.ng-knowledge-header` above the tabs is deleted; v1.76.0 deleted the drill pane's duplicate hero bar) — the app's ONLY progress meter. **Nothing is gated by the score** and the thresholds are provisional.
