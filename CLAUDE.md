...
- **Pane bottom anchor (v1.93.0; final stack v1.98.1):** the guest save nudge is a three-level unit — muted caption `[data-anchor-caption]` "Save your progress", full-width primary `[data-anchor-auth]` "Create account" (44px, the block's one visual anchor), centered quiet escape line `[data-anchor-alt]` "Already have one? Log in" (44px link) — ONE block (`.ng-pane-anchor`, `[data-anchor-auth]`/`[data-anchor-login]`) at the pane's foot, directly above the Settings/Terms/Privacy row — visible on all three tabs, hidden during study takeover and collapsed entirely when signed in (it is then contentless). The mastered/today/weak-spots stat row moved to the TOP of Explore's body (`_exploreStatsRow`, `[data-explore-stats]`, same `.ngStat` handles) — "the weak spots are the call to action" there (owner, v1.95.0). The History head is empty (the drill head is study-only now); signed-in users keep their session CTA + Log out in the Last rolls foot.
