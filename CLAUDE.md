...
- **Pane bottom anchor (v1.93.0):** the mastered/today/weak-spots stat row + the guest save nudge ("Create an account to save your progress" over a quieter "or log in") are ONE block (`.ng-pane-anchor`, `[data-anchor-auth]`/`[data-anchor-login]`) at the pane's foot, directly above the Settings/Terms/Privacy row — visible on all three tabs, hidden only during study takeover. The History head is empty (the drill head is study-only now); signed-in users keep their session CTA + Log out in the History foot.
