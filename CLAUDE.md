...
**THE BOTTOM THUMB BAND (v1.81.4).** Three fixed tenants at `bottom:28px` on a phone: legend (left), `.ng-transport` (centred), `.ng-drilltab` (right, z-index 6 — *above* the transport's 4). The share cue's buttons widen the pill leftward until it covers play/pause + restart, so `_renderShareCue()` stamps **`body[data-share-cue]`** and the mobile CSS steps the transport aside (`translateX(calc(-50% - 66px))`) and drops the pill's own short label. Deliberately **not** a `max-width` on the pill: a capped right-anchored flex row pushes its last button off the right edge instead of shrinking. Asserted geometrically (transport, cue, legend) at 390x844.
