...
| `npm run e2e` | **The PERMANENT push gate**: 69 deterministic Playwright journeys (`e2e/journeys/`, config `e2e/playwright.config.ts`) against the built site on :8123. Rigged RNG + simulated-time pump via the `journey()` DSL (`e2e/dsl.ts`); workers=1, retries=0, ~35 min. `pree2e` checks the RNG seam. |
