import { categoryNotes } from "./fixtures.js"
...
export function mountCatalog({ kind, items, version }) {
  ...
  if (!items.some((entry) => entry.id === state.selectedId)) state.selectedId = items[0].id
  ...
  function renderNavigation() {
    ...
    const visible = items.filter(
      (entry) =>
        !query ||
        `${entry.title} ${entry.group} ${entry.description}`.toLowerCase().includes(query),
    )
    refs.count.textContent = `${visible.length} of ${items.length} ${kind}`
    if (!visible.length) {
      refs.nav.innerHTML = '<div class="catalog-empty">No matching states.</div>'
      return
    }
  ...
  function sizeFrame() {
    const device = devices.find((entry) => entry.id === state.device) || devices[0]
    ...
    const availableHeight = Math.max(380, Math.min(720, window.innerHeight - 220))
  ...
  function renderSelection({ allowPreferredDevice = false } = {}) {
    ...
    if (!entry.variants.includes(state.variant)) state.variant = entry.variants[0]
    ...
    refs.variant.innerHTML = entry.variants
      .map(
        (variant) => `<option ${variant === state.variant ? "selected" : ""}>${variant}</option>`,
      )
      .join("")
    refs.mobileItem.innerHTML = items
      ...
      .join("")
    refs.preview.innerHTML = entry.render(state.variant)
  ...
  refs.fullscreen.addEventListener("click", () => {
    ...
    refs.fullscreen.textContent = state.fullscreen ? "Exit full frame" : "Full frame"
    requestAnimationFrame(sizeFrame)
  })
