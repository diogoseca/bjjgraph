...
The main content workflow is **`npm run regenerate:build`**: regenerate content and derived
data, then build the site. Use the smaller commands when working on a specific layer:

| Task | Command |
|---|---|
| Full regeneration and site build | `npm run regenerate:build` |
| Build the site from current content | `npm run build` |
| Serve the built site | `npm run serve` |
| Refresh the app after UI changes | `npm run dev:neural:app` |
| Run unit checks | `npm run test:units` |
| Run the deployment browser checks | `npm run test:curated` |

The other `regenerate:*` commands are individual pipeline stages; `validate:*` commands cover
separate data and deployment contracts. Keep those entrypoints even when they are rarely run
by hand. For a visible test browser, use `npm run e2e -- --headed`. Redirects, cache headers and
`llms.txt` are already generated by `build`; their Python scripts remain available directly.
