...
const mcState = (page: any) =>
  page.evaluate(() => {
    const a = (window as any).__neural
    return a._mc ? { key: a._mc.key, qhash: a._mc.qhash, correct: a._mc.correct } : null
  })

test("fresh card renders 4 MC options, clamped, with the mc_shown beat", async ({ page }) => {
...
test("MC options are deterministic under rig across two boots", async ({ page }) => {
  const j = journey(page)
  const run = async () => {
    await j.boot("/")
    await j.land("Mount Top")
...
test("wrong answer: no credit, no stage, correct revealed", async ({ page }) => {
  const j = journey(page)
  await j.boot("/")
  await j.land("Mount Top")
  await openLessonDrill(j, page)
  ...
  const before = await page.evaluate((k) => (window as any).__neural.prep[k] || 0, LESSON1.deckKey)
  await page.locator("[data-mc-opt]").nth(wrongIdx).click()
  await j.expectBeat("mc_wrong")
  const after = await page.evaluate((k) => (window as any).__neural.prep[k] || 0, LESSON1.deckKey)
  ...
  expect(
    await page.locator("[data-mc-opt]").nth(truth!.correct).getAttribute("data-mc-result"),
  ).toBe("correct")
})

test("correct answer: full credit + stage, graduating to recall at stage 2", async ({ page }) => {
  const j = journey(page)
  await j.boot("/")
  await j.land("Mount Top")
  await openLessonDrill(j, page)
...
test("mastered only via recall: MC caps at the gate, recall mints rec", async ({ page }) => {
  const j = journey(page)
  await j.boot("/")
  await j.land("Mount Top")
  await openLessonDrill(j, page)
  ...
  s = await page.evaluate(
    ([k, q]) => {
      const a = (window as any).__neural
      return { stage: (a.stage[k] || {})[q] || 0, rec: a.rec[k] || 0, mastered: a.masteredCount() }
    },
    [LESSON1.deckKey, qh] as const,
  )
...
test("graded tiers: plausible = no credit + close feedback, trap = stage penalty", async ({ page }) => {
  const j = journey(page)
  await j.boot("/")
  await j.land("Mount Top")
  ...
  const prep0 = await page.evaluate((k) => (window as any).__neural.prep[k] || 0, LESSON1.deckKey)
  ...
  expect(await page.evaluate((k) => (window as any).__neural.prep[k] || 0, LESSON1.deckKey)).toBe(prep0)
...
test("mcMode setting: classic kills MC; mc still caps the stage at 2", async ({ page }) => {
  const j = journey(page)
  await j.boot("/")
  await j.land("Mount Top")
  ...
  const capped = await page.evaluate((dk) => {
    const a = (window as any).__neural
    const card = a.flashcards.decks[dk].cards.find((c) => a.mcClip(c.a) && a.mcDistractors(c, dk))
...
test("keyboard digits answer MC and never leak into the roll options", async ({ page }) => {
  const j = journey(page)
  await j.boot("/")
  await j.land("Mount Top")
  await openLessonDrill(j, page)
...
test("a11y: radiogroup semantics and a live result region", async ({ page }) => {
  const j = journey(page)
  await j.boot("/")
  await j.land("Mount Top")
  await openLessonDrill(j, page)
...
test("one-beacon law: MC never claims the beacon", async ({ page }) => {
  ...
  await j.boot("/")
  await j.land("Mount Top")
  const beacon0 = await page.evaluate(() => (window as any).__neural.beaconState()?.target || null)
  await openLessonDrill(j, page)
  await presentMcCard(page)
  const truth = await mcState(page)
  await page.locator("[data-mc-opt]").nth(truth!.correct).click()
  await j.advance(700)
  const beacon1 = await page.evaluate(() => (window as any).__neural.beaconState()?.target || null)
...
test("recall fallback: a card with <2 viable distractors renders classic recall", async ({ page }) => {
  const j = journey(page)
  await j.boot("/")
  await j.land("Mount Top")
  ...
  await page.evaluate((dk) => {
    ...
    for (let i = 1; i < deck.cards.length; i++) deck.cards[i] = { q: deck.cards[i].q, a: correct + " " }
...
test("checkpoint is now a real MC quiz: pass marks the unit", async ({ page }) => {
  const j = journey(page)
  ...
  await j2.boot("/", { initialState: { v: 2, prep, rec: { ...prep }, stage: {}, units: {}, belts: { won: {} }, days: {}, settings: {} } })
...
test("checkpoint fail links the weakest lesson", async ({ page }) => {
  ...
  await j.boot("/", { initialState: { v: 2, prep, rec: { ...prep }, stage: {}, units: {}, belts: { won: {} }, days: {}, settings: {} } })
  await j.land("Mount Top")
  ...
  for (let i = 0; i < UNIT1.checkpoint.cards; i++) {
    const t = await mcState(page)
    if (!t) break
    await page.locator("[data-mc-opt]").nth((t.correct + 1) % 4).click()
    await j.advance(700)
  }
  ...
  const failBeat = (await j.beats()).filter((b: any) => b.beat === "checkpoint_failed").pop() as any
