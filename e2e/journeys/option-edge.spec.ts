...
const cardEdges = (page: any) =>
  page.evaluate(() =>
    [...document.querySelectorAll("[data-tech]")].map((c: any) => ({
      ...
      mid: (c.querySelector("span[style*='letter-spacing:.16em']") || {}).textContent || "",
      edge: c.querySelector(".ngedge") ? c.querySelector(".ngedge").textContent : null,
      edgeCol: c.querySelector(".ngedge") ? c.querySelector(".ngedge").style.color : null,
      glyphFilter: c.querySelector(".ngglyph") ? c.querySelector(".ngglyph").style.filter : null,
...
test("@curated at the authored odds the card IS the published value — Frame -12, Escape +18", async ({
  ...
  for (const r of rows) if (r.e0 != null) expect(r.edge, `${r.t} at rest`).toBe(r.e0)
  ...
  expect(frame, "Frame from Side Control is dealt from side-control/bottom").toBeTruthy()
  expect(frame.edge, "the most-attempted move from here is the worst card on the board").toBe(-12)
  ...
  const anchor = await page.evaluate(() => {
    ...
    for (const n of a.nodes) {
      ...
      return { gi: br.gi, frame: n.cal.successRate, p0: a._evP0(n), cal: a.calSuccess(n) }
    }
    return null
  })
  expect(anchor, "a gi-divergent node exists to test the anchor on").toBeTruthy()
  expect(anchor!.cal! * 100, "calSuccess follows the ACTIVE ruleset").toBeCloseTo(anchor!.gi, 6)
  expect(anchor!.p0! * 100, "p0 follows the SOLVE's frame instead").toBeCloseTo(anchor!.frame, 6)
  expect(anchor!.p0, "and the two really do differ on this node").not.toBeCloseTo(anchor!.cal!, 6)
})
...
test("@curated the baseline moves with the hand: EDGE stays a DIFFERENCE at live odds", async ({
  ...
  expect(Math.abs(m.shift), "the opponent really is shifting these odds").toBeGreaterThan(0.15)
  expect(Math.abs(m.wmean), "Σ att·EDGE / Σ att == 0 — that is what zero MEANS").toBeLessThan(1.5)
  expect(m.positives, "a hand cannot be entirely worse than its own average").toBeGreaterThan(0)
})
...
test("@curated the submissions constant is gone: the hand is ranked, not alphabetical", async ({
  ...
  const subsInOrder = await page.evaluate(() => {
    ...
    const subs = t.filter(
      (x: string) => (a.nodes.find((n: any) => n.t === x) || {}).ty === "submissions",
    )
    return subs.join("|") === subs.slice().sort().join("|") && subs.length >= 5
  })
  expect(subsInOrder, "submissions are no longer dealt in alphabetical order").toBe(false)
  ...
  for (const k of keyed)
    expect(k.ord, `${k.t}: the sort key IS the EDGE the card prints`).toBe(k.edge)
  ...
  for (let i = 1; i < valued.length; i++)
    expect(valued[i], `card ${i} ranks below card ${i - 1}`).toBeLessThanOrEqual(valued[i - 1])
  const firstNull = vals.indexOf(null)
  if (firstNull >= 0) expect(vals.slice(firstNull).every((v: any) => v == null)).toBe(true)

  expect(titles[0], "the hand opens on its best-EDGE card").toBe("Kimura from Mount")
})

test("@curated a JIT grade moves the numbers and NEVER the order", async ({ page }) => {
  const j = journey(page)
  await j.boot("/")
  await j.land("Mount Top")
  ...
  expect(after.map((c: any) => c.tech).join("|"), "the tray did not re-sort").toBe(order0)
  expect(
  ...
  expect(parseInt(now.edge, 10), "the drilled card's EDGE moved").toBeGreaterThan(
    parseInt(e0, 10),
  )
})

test("three marks, two channels: the glyph wears the corner number's colour", async ({ page }) => {
  const j = journey(page)
  await j.boot("/")
  await j.land("Mount Top")
  const cards = await cardEdges(page)
...
test("a move the table cannot value shows NO number — never a fabricated 0", async ({ page }) => {
  const j = journey(page)
  await j.boot("/")
  await landBottom(j, page, "Side Control Top")
  ...
  const probe = await page.evaluate(() => {
    ...
    const zero = opts.filter((o: any) => o.ev && a.edgeMark(o) && a.edgeMark(o).i === 0)
    return {
  ...
  expect(probe.unvalued.length, "this hand really does deal cards the table cannot value").toBeGreaterThan(0)
  for (const v of probe.unvaluedEdge) expect(v, "no value means null, not 0").toBeNull()
  expect(probe.zeroSomewhere, "a genuinely-zero EDGE exists and prints").toBeTruthy()
  expect(probe.zeroSomewhere!.txt).toBe("0")
})

test("the sheet's enlarged head cannot contradict the card it grew from", async ({ page }) => {
  const j = journey(page)
  await j.boot("/")
  await j.land("Mount Top")
