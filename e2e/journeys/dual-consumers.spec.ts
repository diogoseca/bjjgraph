...
test("@curated the EDGE node-index join survives the split: every card prints the same number", async ({
  ...
  expect(legacy.nodes, "the noPairs control group is one node per site").toBe(1448) // census:sites
  expect(pair.nodes, "the default is two members per site").toBe(2896) // census:members
  expect(pair.reps, "…over the same live sites").toBe(legacy.reps)

  const keys = Object.keys(legacy.hands)
  expect(keys.length, "every distinct position hand is dealt on both graphs").toBe(242) // census:positionChoiceSeats
...
test("@curated the hub keeps its identity: ordinals, Explore, deck keys, curriculum, names", async ({
  ...
  expect(Object.keys(pair.ordinals).length, "live nodes carry a share ordinal").toBe(1448) // census:ordinals
  ...
  expect(pair.explorer.Positions.length, "distinct positions, excluding submission aliases").toBe(121) // census:playablePositions
  ...
  expect(pair.traffic.length, "the weighted start pool is still 133 POSITIONS").toBe(133) // census:positions
...
test("the hand is identical from either half of a site", async ({ page }) => {
  ...
  expect(r.same, "…over all 266 role-hands").toBe(266) // census:roleHands
})
...
test("@curated every technique retains its origin metadata, and submission states offer their own finish", async ({
  ...
  expect(r.stageTy.positions, "all technique members retain their physical origin").toBe(2630) // census:techMembers
  ...
  expect(r.inHand + r.projected, "every technique is dealt or explicitly projected from a retired control alias").toBe(1315) // census:techSites
})
...
test("both perspective pages of a technique resolve, and the Defender page plays the defender", async ({
  ...
  expect(r.att.n, "over every technique page in the corpus").toBe(1314) // census:techPages
})
