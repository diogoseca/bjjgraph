...
test("@curated the EDGE node-index join survives the split: every card prints the same number", async ({
  ...
  expect(legacy.nodes, "the noPairs control group is one node per site").toBe(1445) // census:sites
  expect(pair.nodes, "the default is two members per site").toBe(2890) // census:members
  expect(pair.reps, "…over the same 1,464 sites").toBe(legacy.reps)

  const keys = Object.keys(legacy.hands)
  expect(keys.length, "every role-hand is dealt on both graphs").toBe(266) // census:roleHands
  ...
  expect(cards, "1,328 dealt cards compared").toBe(1312) // census:dealtCards
...
test("@curated the hub keeps its identity: ordinals, Explore, deck keys, curriculum, names", async ({
  ...
  expect(Object.keys(pair.ordinals).length, "1,464 live nodes carry a share ordinal").toBe(1445) // census:ordinals
  ...
  expect(pair.explorer.Positions.length, "133 position families, once each").toBe(133) // census:positions
...
test("@curated every technique seats a roll at its canonical origin, on the side that performs it", async ({
  ...
  expect(r.stageTy.positions, "all 2,656 technique members seat at a position").toBe(2624) // census:techMembers
  ...
  expect(r.inHand, "1,328 of 1,328 techniques are dealt from where they seat you").toBe(1312) // census:techSites
})
...
test("both perspective pages of a technique resolve, and the Defender page plays the defender", async ({
  ...
  expect(r.att.n, "over every technique page in the corpus").toBe(1311) // census:techPages
})
