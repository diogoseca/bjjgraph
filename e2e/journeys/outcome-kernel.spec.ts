...
test("@curated every node's rolled outcome distribution IS its authored one, drawn inside the branch the gate chose", async ({ page }) => {
  ...
  expect(r.nodes, "every node carrying cal.outcomes was swept").toBe(1331) // census:calOutcomeNodes
  ...
  expect(r.endsCounter, "outcome lists ending in a counter — why the .find() drained counters").toBe(1327) // census:endsCounter
