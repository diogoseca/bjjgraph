 ...
 *   they press play and only then does a session begin → they open the Belt Path and find the
 *   belt bar at zero, the tutorial counting their real progress → it all survives a reload.
 *
...
test("newcomer's first session: coach → question → execute → pane → roam → path → persists", async ({
  page,
}) => {
  ...
  const clock0 = await page.evaluate(() => (window as any).__neural.decisionRemaining())
  await j.advance(3000)
  expect(
    Math.abs((await page.evaluate(() => (window as any).__neural.decisionRemaining())) - clock0),
  ...
  const target = await page.evaluate(() => {
    ...
    for (const i of a.optionIdxs || []) {
      ...
      if (a.nodes[i].ty === "transitions" && odds >= 20 && odds <= 70) return a.nodes[i].t
    }
  ...
  expect(await j.displayedOdds(target), "odds rose").toBeGreaterThan(oddsBefore)
  expect(
  ...
  await expect(page.locator("[data-go]"), "the sheet, with what it wins you").toBeVisible()
  ...
  expect(await page.evaluate(() => !!(window as any).__neural.paused), "roll live").toBe(false)
  await page.locator(".ng-drilltab").click()
  expect(await page.evaluate(() => !!(window as any).__neural.paused), "the pane stopped it").toBe(
    true,
  )
  await j.expectBeat("pane_paused")
  expect(await page.locator("[data-hist]").count(), "a row per state visited").toBeGreaterThan(1)
  ...
  await expect(page.locator(".ng-drill [data-mc-opt]"), "never options in here").toHaveCount(0)
  await page.locator(".ngClose").first().click()
  expect(await page.evaluate(() => !!(window as any).__neural.paused), "closing resumed it").toBe(
    false,
  )
  ...
  const elsewhere = await page.evaluate(() => {
    ...
    for (const n of a.nodes) {
      ...
      if (a.adj[n.idx].some((k: number) => a.nodes[k].ty !== "positions")) return n.idx
    }
    return -1
  })
  await page.evaluate((i) => (window as any).__neural.stageRollAt(i), elsewhere)
  ...
  expect(await page.evaluate(() => !!(window as any).__neural.paused), "and held").toBe(true)
  expect(await page.evaluate(() => !!(window as any).__neural._played), "nothing spent yet").toBe(
    false,
  )
  ...
  // ── 7. the Belt Path: a meter at the bottom of the ladder, tutorial counting real progress ──
  await page.evaluate(() => {
    const a = (window as any).__neural
    a.setViewMode("path")
    a.openExplorer()
  ...
  await expect(page.locator("[data-belt-track]"), "the belt bar").toBeVisible()
  expect(await page.locator("[data-belt-mark]").count()).toBe(5)
  expect(
    await page.locator("[data-belt-fill]").getAttribute("data-belt-fill"),
    "a newcomer has met no belt",
  ).toBe("none")
  await expect(
    page.locator("[data-tut-row]"),
    "the tutorial rides at the head of the path",
  ).toBeVisible()
  const tutDone = await page.evaluate(() => (window as any).__neural.tutDoneCount())
  expect(tutDone, "and it has been ticking off what they actually did").toBeGreaterThan(3)
  expect(await page.locator("[data-crown]").count(), "every lesson wears a crown").toBeGreaterThan(
    10,
  )
  ...
  expect(back.tut, "tutorial progress persisted").toBe(snapshot.tut)
  ...
  await expect(page.locator("[data-coach]"), "no second first-run").toHaveCount(0)
