/* @hyperspace {"theme":"lifetime-journeys","L":"multi-belt-endgame","F":"opponent-turns","B":"happy-path"} @invariant "Outside a content capstone the opponent's move pool is the full graph, not a track-restricted set: for a completed player a rigged opponent_move in a normal roll resolves to a transitions-or-submissions node whose technique need not be in any track's names list, and no _beltTest is ever set — track vocabulary restriction is capstone-only." */
import { test, expect } from "@playwright/test"
import { journey } from "../dsl"
import { multiBeltEndgame, CURRICULUM } from "./personas"

/**
 * ENDGAME OPPONENT VOCABULARY IS UNBOUNDED — the content-capstone flow proves the opponent is
 * RESTRICTED to a track's computed pool DURING a boss battle. This is the exact mirror: in a
 * NORMAL roll (no content capstone) the opponent draws from the WHOLE graph. Track-vocabulary
 * restriction is capstone-only; a completed player rolling casually
 * faces the full move set.
 *
 * Mechanism under test (neural/src/app.src.jsx):
 *   - _beltPoolAllows(n): `const bt=this._beltTest; if(!bt) return true;` — with no internal
 *     capstone test active it returns true for EVERY node; the track `names` list is consulted ONLY
 *     when _beltTest is set. This one guard is the whole invariant.
 *   - startRoll forces `this._beltTest = null` on every normal roll, so no ambient capstone
 *     state can leak in from the persona.
 ...
 *
 * RIG: capstones force the pool-restricted finish branch; we flip the finish draw HIGH to force
 * the unrestricted counter branch:
...
/** union of every track's computed pool (lowercased base names), both rulesets — the vocabulary
 *  an internal capstone test WOULD confine the opponent to. Built from the served fixture. */
...
test("normal-roll opponent turn: move drawn from the full graph, no capstone-pool restriction, no _beltTest", async ({
  page,
}) => {
  // curriculum facts the census leans on — fail loudly here if the corpus shifts
  expect(BELTS.length, "curriculum defines content tracks").toBeGreaterThan(0)
  expect(BELTS.every((b: any) => b.pool), "every track defines a computed pool").toBe(true)
  expect(POOL_UNION.size, "the track-pool vocabulary union is non-empty").toBeGreaterThan(0)
  ...
  // ── this is a NORMAL roll, not a content capstone: startRoll forced _beltTest=null. ──
  expect(await page.evaluate(() => (window as any).__neural._beltTest ?? null), "no capstone before the turn").toBe(
    null,
  )
  expect((await beatsOf(j, "belt_test_start")).length, "no content capstone was ever started").toBe(0)
  ...
  // _beltPoolAllows()-eligible right now. Inside a capstone names.indexOf would gate these out;
  ...
  // proven at the guard level (a capstone would leave outOfPoolAllowed at 0).
  expect(census.outOfPool, "opponent adjacency includes techniques outside every track's pool").toBeGreaterThan(0)
  expect(
    census.outOfPoolAllowed,
    "every out-of-pool candidate is capstone-pool-eligible (guard short-circuits true with no _beltTest)",
  ).toBe(census.outOfPool)
  expect(census.beltTest, "still no content capstone at census time").toBe(null)

  // ── force the opponent turn down the POSITIONAL-COUNTER branch (unrestricted), not the
  //    finish branch (the only branch a content capstone would pool-restrict) ──
  ...
  // the chosen move is pool-eligible via the same guard a capstone would use — true here
  // BECAUSE no content capstone is active (the guard's short-circuit), not because it is in-pool
  ...
  // ── _beltTest stayed null through the entire opponent turn: this was never capstone-gated ──
  expect(await page.evaluate(() => (window as any).__neural._beltTest ?? null), "no content capstone after the turn").toBe(
    null,
  )
  expect((await beatsOf(j, "belt_test_start")).length, "no content capstone started during the turn").toBe(0)
})
