...
test("spa nav single-instance law: two soft navs → one root, fresh instance each life, one live keydown handler", async ({
  page,
}) => {
  test.skip(
    !LESSON1?.deckKey,
    "curriculum lost white unit-1 lesson-1 — MC keydown leg premise gone",
  );
  ...
  const rootCount = () =>
    page.evaluate(() => document.querySelectorAll("#neural-root").length);
  const beatCount = (name: string) =>
    page.evaluate(
      (n) =>
        ((window as any).__neural.beats || []).filter((b: any) => b.beat === n)
          .length,
      name,
    );
  ...
  expect(
    await page.evaluate(() =>
      (window as any).__kdSet.has((window as any).__life1Key),
    ),
  ...
  expect(
    life2.isNewInstance,
    "window.__neural is a NEW object — life 1 not recycled",
  ).toBe(true);
  expect(
    life2.oldDestroyed,
    "life-1 instance ran destroy() (__ngDestroyed set)",
  ).toBe(true);
  expect(life2.oldRootDetached, "life-1 #neural-root left the DOM").toBe(true);
  expect(
    life2.beatsLen,
    "fresh instance: beats array EMPTY right after remount",
  ).toBe(0);
  expect(life2.nodesLen, "fresh instance: new nodes ingested").toBeGreaterThan(
    0,
  );
  expect(
    life2.oldKeyInLedger,
    "life-1 keydown handler REMOVED (no accumulation)",
  ).toBe(false);
  expect(life2.newKeyInLedger, "life-2 keydown handler registered").toBe(true);
  expect(
    life2.newKeyIsOldKey,
    "life-2 handler is a distinct function, not the old one",
  ).toBe(false);
  await page.evaluate(() => {
    const w = window as any;
  ...
  await page.evaluate(() =>
    (window as any).spaNavigate(new URL("/", location.origin)),
  );
  await waitRemount(2);
  ...
  expect(
    life3.roots,
    "exactly one #neural-root after two spa navigations",
  ).toBe(1);
  expect(life3.beatsLen, "life-3 beats array EMPTY right after remount").toBe(
    0,
  );
  expect(life3.nodesLen, "life 3 ingested nodes fresh").toBeGreaterThan(0);
  expect(life3.key1InLedger, "life-1 handler still absent after nav 2").toBe(
    false,
  );
  expect(
    life3.key2InLedger,
    "life-2 handler removed by nav 2 (no accumulation)",
  ).toBe(false);
  expect(
    life3.liveKeyInLedger,
    "exactly the live life-3 handler is registered",
  ).toBe(true);
  ...
  expect(stream, "life-3 stream contains one clean landing").toEqual(
    expect.arrayContaining(["stakes", "land", "options_dealt", "beacon_moved"]),
  );
  expect(
    stream.some((beat) => beat.startsWith("coach_")),
    "coach dismissal persists across soft navs",
  ).toBe(false);
  expect(
    await beatCount("land"),
    "exactly 1 land beat on life 3 — a double mount would double it",
  ).toBe(1);
  ...
  expect(await rootCount(), "still exactly one #neural-root at the end").toBe(
    1,
  );
});
