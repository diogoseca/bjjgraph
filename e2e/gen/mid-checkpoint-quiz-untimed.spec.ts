...
test("checkpoint quiz is untimed: 45s pump fires no expiry beats, i moves only on answers, quiz stays live", async ({ page }) => {
  // curriculum facts the arc leans on
  expect(U2.checkpoint && U2.checkpoint.cards, "unit 2 defines a checkpoint quiz").toBeGreaterThan(1)
  ...
  const pre = await page.evaluate(() => {
    ...
    return { hasDecision: !!a._decision, total: a._decision ? a._decision.total : 0, optPick: !!a._optPick, coach: !!a._coach, paused: !!a.paused }
  })
  expect(pre.hasDecision, "a decision window is armed before the quiz").toBe(true)
  expect(pre.optPick, "the hand is unpicked").toBe(true)
  expect(pre.coach, "coach dismissed — nothing else freezes the clock").toBe(false)
  expect(pre.paused, "game unpaused pre-explorer").toBe(false)
  expect(pre.total, "the window is finite (expiry is reachable well within the pump)").toBeLessThan(30000)

  // ── open the rail and start unit 2's checkpoint ──
  ...
  for (let k = 0; k < 45; k++) {
    ...
    seen = await page.evaluate((base) => {
      const names = (((window as any).__neural || {}).beats || []).map((b: any) => b.beat)
      return {
        warn: names.filter((n: string) => n === "expiry_warning").length - base.warn,
  ...
  expect(seen.warn, "zero expiry_warning while the checkpoint quiz is open").toBe(0)
  ...
  expect(after.i, "_checkpoint.i unchanged by time — only answers move it").toBe(snap.i)

  // liveness: the quiz card is still answerable — the NEXT correct answer advances i
  expect(after.mcOptCount, "quiz MC options still on screen after the pump").toBeGreaterThanOrEqual(2)
  ...
  await j.advance(300)
  const i2 = await page.evaluate(() => ((window as any).__neural._checkpoint || {}).i ?? null)
  expect(i2, "answering still advances the quiz — the card the user left is the card they resume").toBe((snap.i as number) + 1)
})
