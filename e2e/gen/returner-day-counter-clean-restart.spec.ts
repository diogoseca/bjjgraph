 ...
 *     dedupes repeats. Blended-hierarchy cards are DUPLICATED across decks, so the study card
 ...
 * their openLessonStudy handler, so lesson 1 reopens for the recall surface. land() rigs the
...
test("lapsed returner: daily counter restarts clean — 0 at boot, study grade → 1, +2 drills → 3, _days single-key throughout", async ({
  page,
}) => {
  ...
  expect(
    boot.wonWhite,
    `persona ingested: belts.won["${WHITE.id}"] present (guards against a silently-fresh boot)`,
  ).toBe(true);
  expect(
    boot.prepDecks,
    "persona ingested: prep carries the returner's drilled decks",
  ).toBeGreaterThan(0);
  expect(boot.cardsToday, "comeback boots at a clean zero").toBe(0);
  expect(
    boot.dayKeys,
    "days map is EMPTY at boot — no stale day survived the lapse",
  ).toEqual([]);

  // ── dedup guard: noteCardDone credits once per question text, and j.drill(2) will grade
  //    the Mount Top deck's cards[0]/[1] — read those questions UP FRONT so the study pick
  ...
  const drillQs: string[] = await page.evaluate(() => {
    ...
    return (a.flashcards.decks[key].cards || [])
      .slice(0, 2)
      .map((c: any) => c.q);
  });
  expect(
    drillQs.length,
    "current-position drill deck has at least 2 cards",
  ).toBe(2);
  expect(drillQs[0], "the two drill targets are distinct questions").not.toBe(
    drillQs[1],
  );
  ...
  expect(
    ...
    "done lesson 1 reopens for study",
  ).toBe(true);

  const qh = await page.evaluate((skip) => {
    const a = (window as any).__neural;
    for (const c of a.deck || []) {
      if (!(skip as string[]).includes(c.q)) {
  ...
  expect(
    qh,
    "a study card outside the drill-target questions exists in lesson 1",
  ).toBeTruthy();
  ...
  expect(
    afterFirst.cardsToday,
    "first graded card makes the counter EXACTLY 1",
  ).toBe(1);
  expect(
    afterFirst.dayKeys,
    "days map holds ONLY today's key after the first credit",
  ).toEqual([afterFirst.today]);
  ...
  expect(
    final.cardsToday,
    "counter is honest across surfaces: 1 study grade + 2 drill = 3",
  ).toBe(3);
  expect(
    final.dayKeys,
    "STILL exactly one day key — the comeback session never grows stale days",
  ).toEqual([final.today]);
  ...
  const pumped = (await j.beats()).filter(
    (b) => b.beat === "bonus_pumped",
  ).length;
