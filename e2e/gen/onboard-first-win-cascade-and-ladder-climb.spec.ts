/* @hyperspace {"theme":"onboarding","L":"first-roll-day1","F":"victory-defeat","B":"cross-feature"} @invariant "A newcomer's first rigged submission win produces the full celebration and their first ladder rung: finish then roll_end outcome 'win', a victory_cascade with durMs<=1500 and ladder_up (+1 rank) from baseline rank 1, and no belt_test_won beat fires (a normal-roll win is not a belt award)." */
 ...
 * (bounded animation), and the FIRST LADDER RUNG — while the belt ledger stays inert, because
 * a plain roll is not a belt test.
 *
 * Structural grounding (neural/src/app.src.jsx, source-verified at authoring):
 *   - endRound(kind, name) @3781: the ENTIRE belt block is guarded by `if (this._beltTest)`
 *     @3784. A plain land("Mount Top") roll has _beltTest === null, so belt_test_won @3791 and
 *     belt_unlocked @3793 never fire on this win.
 ...
 *   - endgame-victory-cannot-double-award-belt.spec.ts: SAME win mechanism, but persona
 ...
 *     durMs<=1500 cascade bound nor the belt-inertness of the win.
 *   Unique claim here: firstRollDay1 at baseline rank 1 — the NEWCOMER first-win cut — asserting a
 *   bounded victory_cascade AND the first rung climb, with the belt block provably inert.
 *
...
test("newcomer first win: finish→roll_end 'win', bounded victory_cascade + first ladder rung, no belt award", async ({ page }) => {
  const errors: string[] = []
  ...
  // ── BASELINE: fresh ladder — rank 1 (default, store absent) and no belt test staged. This is
  //    the newcomer premise the +1 climb and the belt-inertness assertions lean on. ──
  ...
  expect(baseline.staged, "a plain land() roll stages no belt test — _beltTest is null").toBe(false)
  const r0 = baseline.rank

  // ── Pre-roll census: no belt-award beats have fired (the persona doesn't replay any) ──
  const preBeats = await j.beats()
  expect(preBeats.filter((b) => b.beat === "belt_test_won").length, "no belt_test_won before the roll").toBe(0)
  expect(preBeats.filter((b) => b.beat === "belt_unlocked").length, "no belt_unlocked before the roll").toBe(0)
  ...
  // ── THE INVARIANT: bounded cascade + first rung climb, belt block inert. Read the beat stream
  ...
  const roll = await page.evaluate(() => {
    ...
    return {
      ...
      unlockedCount: beats.filter((b) => b.beat === "belt_unlocked").length,
  ...
  // belt block INERT: a normal-roll win is not a belt award — no belt beats, no staged test
  expect(roll.wonCount, "no belt_test_won on a normal-roll win — the belt block is gated out").toBe(0)
  expect(roll.unlockedCount, "no belt_unlocked on a normal-roll win").toBe(0)
  expect(roll.staged, "_beltTest still null after the win — the belt block never ran").toBe(false)
