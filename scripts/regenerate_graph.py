...
def process_systems(content_dir: Path, node_index: dict | None = None) -> dict:
    ...
    is verified course metadata, passed through without URLs (see below).
    """
    ...
    for slug, entry in systems.items():
        ...
        if isinstance(products, list) and products:
            # Committed graph products retain metadata only. No course/referral URLs are
            # needed by graph consumers. Keep the legacy presence flag for compatibility.
            entry['products'] = [
                dict(
                    {k: v for k, v in p.items() if k not in ('affiliate_url', 'course_url', 'url')},
                    has_affiliate_url=bool(p.get('course_url')),
