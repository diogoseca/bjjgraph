...
- [~] **Epic B (phases 5-8)** — graph restructure (collapse role nodes, per-role strength [-1,1], red↔white↔blue ramp, node-type shapes). High blast radius; own focused effort.
  ...
  - [x] **Phase 7+8 — background graph (the hero) colour + shapes** — DONE (uncommitted, riding on the user's WIP per "one combined diff"). `backgroundGraph.inline.ts`: `rampColor`/`strengthColor`/`drawNode` (positions=circle, transitions=rounded-rect, submissions=circle+`--light` "game-over" outline); reads `node.s` from the enriched layout; `currentRole` from `<body data-current-role>` (added in `renderPage.tsx`), recoloured on every SPA nav via `highlightCurrentNode`. Ramp vars added to `quartz.config.ts` (both modes) + `theme.ts` (`ColorScheme` + CSS-var emission). Existing `graph*` vars KEPT (local graph still uses them). tsc + prettier green.
  - [ ] **FOLLOW-UP A — local sidebar graph (`graph.inline.ts`)** — still on category colours. Needs strength plumbed through the `contentIndex` emitter + `ContentDetails` (plan Phase 9), since it reads `window.fetchData`, not `graph.json`. Separable.
  - [ ] **FOLLOW-UP B — legend overlay** — deferred to the user's design (they're mid-redesign of the drawer aesthetic; the legend look should match it). Spec in plan §6.8.
  - [ ] **dark-mode submission outline nuance** — uses `--light` (= bg colour → "portal cutout" ring, reads white in light mode, dark in dark mode) per plan §6.8; user wanted literal "white" — confirm in visual review.
