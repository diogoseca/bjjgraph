...
function addGlobalPageResources(ctx: BuildCtx, componentResources: ComponentResources) {
  ...
  if (cfg.analytics?.provider === "google") {
    ...
    componentResources.afterDOMLoaded.push(`
      ...
      // Bug report button: trigger PostHog survey on click (SPA-safe via delegation)
      document.addEventListener("click", function(e) {
        var btn = e.target.closest(".bug-report")
        if (!btn || !window.posthog) return
        e.preventDefault()
        window.posthog.getActiveMatchingSurveys(function(surveys) {
          var s = surveys.find(function(s) { return s.name === "Page Bug Report" })
          if (s) window.posthog.renderSurvey(s.id)
        })
      })
    `)
