...
<!--
TRANSITION_SUMMARY: Hip Bump Sweep (T045) - Fundamental sweep from Closed Guard Bottom to Mount. Low complexity, high reliability technique suitable for all skill levels. Beginner success 50%, scales to 85% at advanced level.

KEY_MECHANICS: Collar grip + posted hand creates lever system. Sit-up motion generates upward force while hip bump creates lateral displacement. Combined forces overwhelm opponent's base structure. Shoulder push completes off-balance.

STRATEGIC_VALUE: Essential first sweep for beginners. Low energy cost (3 units attacker vs 8 defender). Quick execution time (2-3 seconds). Low risk of position loss. Chains well with submission attempts.

COUNTER_PATTERNS: Base out (55% if telegraphed), posture up (45% vs strong posture), weight forward (40% timing-dependent), counter sweep (30% requires superior timing).

SUCCESS_FACTORS: Setup quality +10%, timing precision +15%, opponent fatigue +5%, knowledge test +10%, position control +10%. Optimal when opponent upright with centered weight. Avoid vs low heavy base.

SYSTEM_INTEGRATION: Core closed_guard_attacks system. Prerequisite for advanced guard work. Chains with triangle, armbar, scissor sweep. Fundamental in Danaher systematic approach, competition staple per Gordon Ryan, integrates with Eddie Bravo rubber guard concepts.
-->
